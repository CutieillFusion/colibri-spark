...
def _k3_i4g_gemv_kernel(
    a_ptr, c_ptr, w_ptr, s_ptr, bias_ptr,
    M, N, K,
    stride_am, stride_ak, stride_wn, stride_sn, stride_cm, stride_cn,
    ...
    for k0 in range(0, K, BLOCK_K):
        ...
        b = tl.load(w_ptr + offs_n[:, None] * stride_wn + (k0 // 2 + jj)[None, :],
                    mask=n_mask[:, None], other=0).to(tl.uint8)
        lo = (b & 0xF).to(tl.float32) - 8.0
        hi = ((b >> 4) & 0xF).to(tl.float32) - 8.0
        xe = tl.load(a_ptr + pid_m * stride_am + ke * stride_ak,
                     mask=ke < K, other=0.0).to(tl.float32)
        xo = tl.load(a_ptr + pid_m * stride_am + (ke + 1) * stride_ak,
                     mask=ke + 1 < K, other=0.0).to(tl.float32)
        sc = tl.load(s_ptr + offs_n[:, None] * stride_sn + (ke // GS)[None, :],
                     mask=n_mask[:, None], other=0.0)
        acc += tl.sum((xe[None, :] * lo + xo[None, :] * hi) * sc, axis=1)

    if HAS_BIAS:
        acc += tl.load(bias_ptr + offs_n, mask=n_mask, other=0.0).to(tl.float32)
    tl.store(c_ptr + pid_m * stride_cm + offs_n * stride_cn,
             acc.to(c_ptr.dtype.element_ty), mask=n_mask)
...
def _k3_i8_gemv_kernel(
    a_ptr, c_ptr, w_ptr, s_ptr, bias_ptr,
    M, N, K,
    stride_am, stride_ak, stride_wn, stride_cm, stride_cn,
    ...
    for k0 in range(0, K, BLOCK_K):
        ...
        k_mask = offs_k < K
        q = tl.load(w_ptr + offs_n[:, None] * stride_wn + offs_k[None, :],
                    mask=n_mask[:, None] & k_mask[None, :], other=0).to(tl.float32)
        x = tl.load(a_ptr + pid_m * stride_am + offs_k * stride_ak,
...
def _k3_dense_gemm_kernel(
    ...
    M, N, K,
    stride_am, stride_ak, stride_wn, stride_sn, stride_cm, stride_cn,
    BITS: tl.constexpr, GS: tl.constexpr, HAS_BIAS: tl.constexpr,
    ...
    for k0 in range(0, K, BLOCK_K):
        ...
        if BITS == 4:
            ...
            sc = tl.load(s_ptr + offs_n[:, None] * stride_sn + (ke // GS)[None, :],
                         mask=n_mask[:, None], other=0.0)
            ae = tl.load(a_ptr + offs_m[:, None] * stride_am + ke[None, :] * stride_ak,
                         mask=m_mask[:, None] & (ke < K)[None, :], other=0.0)
            ao = tl.load(a_ptr + offs_m[:, None] * stride_am
                         + (ke + 1)[None, :] * stride_ak,
                         mask=m_mask[:, None] & (ke + 1 < K)[None, :], other=0.0)
    ...
    if HAS_BIAS:
        acc += tl.load(bias_ptr + offs_n, mask=n_mask, other=0.0)[None, :].to(tl.float32)
...
def k3_dense_matmul(x, qweight, scales, bits, bias=None, group_size=I4_GROUP,
                    block_n=64, block_k=128, num_warps=4):
    ...
    if M <= GEMV_MAX_TOKENS:
        ...
        if bits == 4:
            _k3_i4g_gemv_kernel[grid](
                a, out, qweight, scales, bias if bias is not None else a,
                M, N, K,
                a.stride(0), a.stride(1), qweight.stride(0), scales.stride(0),
                out.stride(0), out.stride(1),
                GS=group_size, HAS_BIAS=bias is not None,
                BLOCK_N=block_n, BLOCK_K=block_k, num_warps=num_warps)
        else:
            _k3_i8_gemv_kernel[grid](
                a, out, qweight, scales, bias if bias is not None else a,
                M, N, K,
                a.stride(0), a.stride(1), qweight.stride(0),
                out.stride(0), out.stride(1),
        ...
        _k3_dense_gemm_kernel[grid](
            ...
            out.stride(0), out.stride(1),
            BITS=bits, GS=group_size, HAS_BIAS=bias is not None,
            BLOCK_M=bm, BLOCK_N=block_n, BLOCK_K=block_k, num_warps=num_warps)
...
def quantize_i4g(w: torch.Tensor, group_size: int = I4_GROUP):
    ...
    O, I = w.shape
    if I % group_size:
        raise ValueError(f"int4-g{group_size} needs I%{group_size}==0, got {I}")
    g = w.float().reshape(O, I // group_size, group_size)
...
def dequant_reference(qweight, scales, bits, group_size=I4_GROUP):
    ...
    v = torch.empty(O, half * 2, dtype=torch.int32, device=qweight.device)

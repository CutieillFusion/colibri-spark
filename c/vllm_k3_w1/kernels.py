...
@triton.jit
def _unpack_signs(packed_ptr, row_off, k_off, stride_o, BLOCK_O: tl.constexpr,
...
def _k3_w1_gemm_kernel(
    a_ptr, c_ptr, packed_ptr, scale_ptr,
    sorted_ids_ptr, expert_ids_ptr, num_tokens_post_pad_ptr,
    topk_weights_ptr,
    ...
    for k0 in range(0, K, BLOCK_K):
        ...
        sign = _unpack_signs(packed_ptr + expert * stride_pe, offs_n, k0,
                             stride_pn, BLOCK_O=BLOCK_N, BLOCK_K=BLOCK_K)
        scl = _load_scales(scale_ptr + expert * stride_se, offs_n, k0,
                           stride_sn, BLOCK_O=BLOCK_N, BLOCK_K=BLOCK_K)
        w = sign * scl                                     # [BN, BK]
...
def _k3_w1_gemv_kernel(
    a_ptr, c_ptr, packed_ptr, scale_ptr,
    topk_ids_ptr, topk_weights_ptr, expert_map_ptr,
    N, K,
    stride_am, stride_ak,
    stride_pe, stride_pn,
    stride_se, stride_sn,
    ...
    for k0 in range(0, K, BLOCK_K):
        offs_k = k0 + tl.arange(0, BLOCK_K)
        xv = tl.load(a_ptr + token * stride_am + offs_k * stride_ak,
                     mask=offs_k < K, other=0.0).to(tl.float32)
        sign = _unpack_signs(packed_ptr + e * stride_pe, offs_n, k0,
                             stride_pn, BLOCK_O=BLOCK_N, BLOCK_K=BLOCK_K)
        scl = _load_scales(scale_ptr + e * stride_se, offs_n, k0,
                           stride_sn, BLOCK_O=BLOCK_N, BLOCK_K=BLOCK_K)
    ...
    tl.store(c_ptr + pid_p * stride_cm + offs_n * stride_cn,
...
def situ_and_mul(x: torch.Tensor, beta: float, linear_beta: float | None):
    ...
    _situ_and_mul_kernel[(flat.shape[0],)](
        out, flat, d, float(beta), -1.0 if linear_beta is None else float(linear_beta),
...
def w1_grouped_gemm(a, packed, scale, sorted_ids, expert_ids, num_tokens_post_pad,
                    topk_weights, out, top_k, amplitude, mul_routed_weight,
...
def w1_gemv(a, packed, scale, topk_ids, topk_weights, expert_map, out, top_k,
            amplitude, mul_routed_weight, block_n=32, block_k=256,

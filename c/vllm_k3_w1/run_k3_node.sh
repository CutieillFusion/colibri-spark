...
ENV="-e PYTHONPATH=/work:/k3w1 -e VLLM_HOST_IP=$MYIP -e MASTER_ADDR=$HEAD_IP -e GLOO_SOCKET_IFNAME=enP7s7 -e VLLM_DISTRIBUTED_TIMEOUT_SECONDS=${K3_DIST_TIMEOUT:-3600} -e TORCH_DISTRIBUTED_TIMEOUT=3600 -e NCCL_SOCKET_IFNAME=enP7s7 -e NCCL_IB_GID_INDEX=3 -e RAY_memory_monitor_refresh_ms=0 -e RAY_gcs_rpc_server_reconnect_timeout_s=900 -e RAY_health_check_timeout_ms=120000 -e RAY_health_check_period_ms=30000 -e RAY_health_check_failure_threshold=20 -e RAY_raylet_heartbeat_timeout_milliseconds=120000 -e K3_W1_DIR=/k3store -e K3_W1_SHARD=$TPR/2 -e K3_W1_STAGE_LOCAL=1 -e VLLM_PP_LAYER_PARTITION=47,46 -e VLLM_PLUGINS=k3_w1 -e PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True -e PYTHONDONTWRITEBYTECODE=1 -e TOKENIZERS_PARALLELISM=false -e K3_MODEL_DIR=$MODEL -e K3_RESIDENT_EXPERTS=${K3_RESIDENT_EXPERTS:-0} -e K3_STREAM_SLOTS=${K3_STREAM_SLOTS:-16} -e K3_QUANT_EMBED=${K3_QUANT_EMBED:-1} -e K3_RESIDENT_EXPERTS=${K3_RESIDENT_EXPERTS:-0} -e K3_STREAM_SLOTS=${K3_STREAM_SLOTS:-16} -e K3_ONE_GPU_PER_NODE=1 -e K3_BITS=${K3_BITS:-4} -e K3_GROUP=${K3_GROUP:-64} -e K3_MLA_BITS=${K3_MLA_BITS:-8} -e K3_HEAD_BITS=${K3_HEAD_BITS:-8} -e K3_HIER_AR=${K3_HIER_AR:-0} -e NCCL_IB_DISABLE=${NCCL_IB_DISABLE:-1}"

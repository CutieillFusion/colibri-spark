...
**3.238 and 3.236 tok/s** on consecutive passes (99.5% expert hits), with a
1.890 tok/s cold-cache pass. Generated bytes matched the previous exact kernel
on every rank and pass. The gain comes from folding the exact int4 GEMV's 256
...
85–103 to 139–155 GB/s. This is a workload working-set result, not a claim that
80 GB can hold all 224 experts per layer; arbitrary prompts may still miss and
stream from NVMe.

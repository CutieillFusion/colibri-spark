...
### CORRECTION: kproj is int4. The dense path is mixed precision, but not there

The previous entry claimed kproj is int8 at 6.07 GB/token and 186 GB/s, and that
this resolved the long-standing 115 GB/s puzzle. **That was wrong.** The census
groups tensors by shape, and two different tensors share the shape
[3072, 7168] after slicing:

* KDA `q/k/v/g`, loaded with `bits` -- default **4** -- so int4-g64;
* MLA `g_proj`, loaded with `mbits` -- default **8** -- so int8,
  because `n_heads * v_head = 96 * 128 = 12288`, the same as the KDA projection
  width, and both shard to 3072 rows.

The census printed both an `fmt=1` and an `fmt=4` line for that shape and I read
the int8 one as kproj. It is not. The engine has three independent precision
knobs, which nothing in this log had noticed:

    bits  = K3_BITS       default 4    KDA q/k/v/g/o, MoE, dense MLP
    mbits = K3_MLA_BITS   default 8    every MLA projection
    hbits = K3_HEAD_BITS  default 8    lm_head

So kproj is int4 at ~3.4 GB/token, its rate is ~105 GB/s, and the puzzle is
**open again** -- three falsified hypotheses and no explanation, still waiting on
`ncu`. `K3_BITS=4` changing nothing is now explained too: 4 is already the
default, and kproj is BF16 in the checkpoint with no `.qs` sidecar, so it never
touches the U8 downcast path that flag gates.

The lesson is narrow and worth keeping: **a census keyed on shape cannot
identify a tensor.** Two tensors of different precision collided on one shape and
produced a confident, wrong conclusion that survived into a commit.

### MLA and lm_head at int4: the GPU wins, the CPU loses more

The real lever the knobs expose is that MLA and lm_head are int8 while everything
else is int4. Halving them looked like ~4%:

| term | int8 (default) | int4 |
|---|---|---|
| mproj (GPU) | 0.67 | **0.501** (-25%) |
| mout | 1.22 | 1.161 |
| matt (CPU absorb) | 0.87 | 1.164 (+34%) |
| head | 0.15 | **0.861** (5.7x) |
| tok/s | **4.563 / 4.559** | 4.389 / 4.356 |

`mproj` improves by exactly what the byte count predicts, so the GPU side works.
But `matt` is the MLA absorb, which runs on the **CPU** through `w_addrow` and
`w_rowdot`, and the int4-grouped CPU path is slower than the int8 one -- it
unpacks nibbles and applies a per-group scale where int8 is a flat multiply. And
`head` gets 5.7x worse, consistent with the sharded lm_head falling off the exact
kernel at int4 and onto a much slower path.

Net loss, so the defaults stand. The general shape is familiar: halving bytes
helps wherever the GPU reads them and hurts wherever a CPU path has to unpack
them, and this model keeps enough attention work on the CPU that the second
effect wins.
